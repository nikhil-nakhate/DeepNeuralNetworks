...
public class CNN {
    ...
    private static boolean isReLU = true; // Feel free to set to a different value.
    ...
    public static void main(String[] args) {
        ...
        //RANDOM SELECTION FOR LEARNING CURVE
        int trainSetSize = trainset.getSize();

        Dataset newTrainSet = new Dataset();
        for (int i = 0; i<trainSetSize; i++) {
            int randomPick  = (int)(Math.random() * (trainSetSize - 1));
            newTrainSet.add(trainset.getImages().get(randomPick));
        }

        trainset = new Dataset();
        for (int lmo = 0;lmo<newTrainSet.getSize() ;lmo++ ) {
            trainset.add(newTrainSet.getImages().get(lmo));
        }
        //System.out.println(String.valueOf(trainset.getSize()));
        ...
        // String [] layerTypes = {"fullyConnected","outputLayer"};
        String [] layerTypes = {"conv","maxpool","fullyConnected","outputLayer"};
        // String [] layerTypes = {"conv","maxpool","conv","maxpool","fullyConnected","outputLayer"};
        int [][] layerDims = {
                {28,28,20},
                {14,14,20},
                // {10,10,20},
        ...
        double alpha = 0.01;
        int maxEpochs = 500;
        int [] inputDims = {32,32,4};
        int numLabels = Category.values().length;
        int tunePeriod = 10;

        ANN cnn = new ANN(layerTypes, layerDims, alpha, maxEpochs, inputDims, numLabels, tunePeriod);
        int counter1 = 0;
        int counter2 = 0;

        // for(ArrayList<Mat3D> filterLayer : cnn.filterLayers) {
        //     System.out.println("Layer: " + counter1);
        //     counter2 = 0;
        //     for(Mat3D filter: filterLayer){
        //         System.out.println("    filter: " + counter2);
        //         System.out.println("        numRows: " + filter.numRows);
        //         System.out.println("        numCols: " + filter.numCols);
        //         System.out.println("        depth: " + filter.depth);
        //         counter2++;
        //     }
        //     counter1++;
        // }
        // counter1 = 0;
        // for(Mat3D actLayer: cnn.activationLayers) {
        //     System.out.println("Layer: " + counter1);
        //     System.out.println("        numRows: " + actLayer.numRows);
        //     System.out.println("        numCols: " + actLayer.numCols);
        //     System.out.println("        depth: " + actLayer.depth);
        //     counter1++;
        // }

        Instance img = trainset.getImages().get(0);

        // cnn.test(trainset,false);
        // img.imageMat3 = new Mat3D(4,4,1);
        // img.imageMat3.UniformInit(1.0);
        // img.imageMat3.Print();

        // cnn.activationLayers.get(cnn.numLayers-1).Print();
        // cnn.feedforward(img);
        // cnn.backprop(img);
        // cnn.updateWeights();
        // System.out.println("----------------------------------------------------");
        // System.out.println("Label = " + img.getLabel());
        // cnn.feedforward(img);
        // cnn.activationLayers.get(cnn.numLayers-1).Print();
    ...
    private static int trainPerceptrons(Vector<Vector<Double>> trainFeatureVectors, Vector<Vector<Double>> tuneFeatureVectors, Vector<Vector<Double>> testFeatureVectors) {
        ...
        for (int i = 0; i < Category.values().length; i++) {
            ...
            for (int indexWgt = 0; indexWgt < inputVectorSize; indexWgt++) perceptron.add(getRandomWeight(inputVectorSize, 1, isReLU)); // Initialize weights.
        }
